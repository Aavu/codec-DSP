...
%   encoded bitrate in bits per second (64000 default)
...
noise = env.*noiseGen(length(Y), 4, 'rect');
noise_frames = enframe(noise, N, N/2);
...
for frame_count=1:length(frames(:,1))
    ...
    fft_frame = fft(frames(frame_count,:).*window);
    ...
    if fft_frame == zeros(1,N)
        ...
        % Temporal Masking
        big_mask_prime = 0.85*big_mask + 0.15*prev_mask;
%         if frame_count == 66
%             plot(prev_mask);
%             hold on;
%             plot(big_mask);
...
%             break;
%         end
        big_mask = max(big_mask, big_mask_prime);
        prev_mask = big_mask;
...
%         Data = noiseShape(original, noise_frames(frame_count,:));
        [b,a] = cheby1(2,0.1,0.75,'high');
        noise = filter(b, a, noise_frames(frame_count,:));
        Data = Data + 2*noise;
        [Gain,Data, err] = p_encode(mdct(Data),Fs,N,bit_alloc,scalebits);
...
% Piece wise spreading function
for i = 1:length(dz)
    if dz(i) < 0
        mask = -(6 + 0.4*spl)*abs(dz(i)) - (11 + 0.4*spl)*(abs(dz(i)) - 1)*theta(abs(dz(i))-1);
    else
        mask = -17*dz(i) + 0.15*spl*(dz(i)-1)*theta(dz(i)-1);
    end
end
...
numBandsToIgnore = 3*floor(64000/bitrate);

half_N = N/2;
num_subbands = floor(fftbark(half_N,half_N,Fs))+1;
bits_per_frame = floor(((bitrate/Fs)*(half_N)) - (scalebits*num_subbands));
        
bits(floor(bark((Fs/2)*(1:half_N)/(half_N))) +1) = 0;

for ii=1:half_N
    bits(floor(bark( (Fs/2)*ii/(half_N) )) +1) = max(bits(floor(bark( (Fs/2)*ii/(half_N) )) +1) , ceil( y(ii)/6 ));
end
...
%        P_ENCODE           %
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function [Quantized_Gain,quantized_words, err]=p_encode(x2,Fs,framelength,bit_alloc,~)
...
err = zeros(size(x2));
for ii=1:numel(x2)
    [quantized_words(ii), err(ii)] = midtread_quantizer(x2(ii), max(bit_alloc(floor(fftbark(ii,framelength/2,Fs))+1),0)+1e-10); % 03/20/03
end

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%    MIDTREAD_QUANTIZER     %
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function [ret_value, err] = midtread_quantizer(x,R)

Q = 2 / (2^R - 1); 
%x_prime = myDither(x, R, 'tri');
q = quant(x,Q);
s = q<0; 
err = x - q;
...
%          Noise Gen        %
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function y = noiseGen(num_samples, w, noise_type)
...
%   Detailed explanation goes here
d = noiseGen(length(x), w, noise_type);
y = x+d;


function y=noiseShape(x,y_dith)
%This function adds dither, quantizes and then noise shapes an audio signal
b = [1 -1.1474 .5383 -.3520 .3475];
a = [1 1.0587 .0676 -.6054 -.2738];
d_y_dith = filter(b,a,y_dith);
